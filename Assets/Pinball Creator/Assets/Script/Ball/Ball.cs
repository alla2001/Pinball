...
public class Ball : MonoBehaviour {
	...
	{
		if(rb.linearVelocity.magnitude > maxSpeed)										// Limit ball speed.
		{
			rb.linearVelocity 			
			= rb.linearVelocity.normalized * maxSpeed;
		}

		if(b_trail && rb.linearVelocity.magnitude > Speed_To_Activate_Trail)				// Enable ball trail.
		{
			trail.enabled=true; 
			b_trail = false;
		}
		if(!b_trail && rb.linearVelocity.magnitude < Speed_To_Activate_Trail)				// Desable ball trail.
		{
			trail.enabled=false; 
			b_trail = true;
		}

		if(rb.linearVelocity.magnitude > min_Mag_roll_audio && once)						// Play the roll sound.
		{
			roll_audio.Play();
			once = false;
		}
		else if(rb.linearVelocity.magnitude <= min_Mag_roll_audio && !once)				// Stop the roll sound.
		{
			roll_audio.Stop();
			roll_audio.pitch = 1;
			once = true;
		...
		if(!once && tmp_Save_Min_Mag == 0){
			roll_audio.pitch = rb.linearVelocity.magnitude/2.5f;							// When ball accelerate the pitch increase. 
		}
	...
	public void Ball_Pause(){													// --> Function Call When the game is on pause
		if(!b_OnHole){
			if(!rb.isKinematic){													// Start Pause
				tmp_vel = rb.linearVelocity;
				rb.isKinematic = true;
			}
			else{																	// Stop Pause
				rb.isKinematic = false;
				rb.linearVelocity = tmp_vel;
			}	
		}	
	}
