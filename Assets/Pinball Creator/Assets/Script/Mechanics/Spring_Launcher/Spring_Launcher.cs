...
public class Spring_Launcher : MonoBehaviour {
	...
	public void BallOnPlunger(Rigidbody rb_obj){										// Know if the ball is on the plunger
		Debug.Log($"BallOnPlunger called! Ball {(rb_obj != null ? "DETECTED" : "LEFT")}");
		...
		if(rb_Ball != null){
			Debug.Log($"Ball stored in rb_Ball: {rb_Ball.name}");
		}
	}
	...
	// VR SUPPORT METHODS
	public void VR_LaunchBall(float vrPullForce){								// --> Add force to the ball using VR pull force
		Debug.Log($"VR_LaunchBall called! vrPullForce: {vrPullForce}, rb_Ball: {(rb_Ball != null ? "EXISTS" : "NULL")}");

		if(rb_Ball != null){
			// Calculate force based on VR pull (vrPullForce is 0-1 normalized)
			float calculatedForce = _Spring_Force * vrPullForce * vrPullForce;

			Debug.Log($"Launching ball! _Spring_Force: {_Spring_Force}, vrPullForce: {vrPullForce}, calculatedForce: {calculatedForce}");
			Debug.Log($"Force direction (forward): {transform.forward}, Total force: {transform.forward * calculatedForce}");

			// Apply force to the ball
			rb_Ball.AddForce(transform.forward * calculatedForce, ForceMode.VelocityChange);

			// Play sound effect
			if(!sound_.isPlaying && Sfx_Kick)
				sound_.PlayOneShot(Sfx_Kick);

			// Change camera if force is strong enough
			if(Cam_Change_Min < vrPullForce){
				if(!b_Debug && camera_Movement)
					camera_Movement.PlayIdle();
				Ball_ExitThePlunger = true;

				// Trigger skillshot mission if available
			...
			Debug.LogWarning("VR_LaunchBall: rb_Ball is NULL! Ball was not detected on plunger. Check Plunger_Ball_Detector.");
		}
	}
